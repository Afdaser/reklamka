...
function wp_bootstrap_starter_posted_on() {
    ...
    $posted_on = sprintf(
        esc_html_x( 'Опубліковано %s', 'post date', 'wp-bootstrap-starter' ),
        '<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
    );

    // Категорії (де саме опубліковано)
    ...
    if ( $categories_list && wp_bootstrap_starter_categorized_blog() ) {
        $posted_in = sprintf(
            esc_html__( ' в %s', 'wp-bootstrap-starter' ),
            $categories_list
        );
    } else {
        $posted_in = '';
    ...
    echo '<span class="posted-on">' . $posted_on . $posted_in . '</span>';
}
endif;
...
function wp_bootstrap_starter_entry_footer() {
	if ( 'post' === get_post_type() ) {
		...
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( 'Теги: %1$s', 'wp-bootstrap-starter' ) . '</span>', $tags_list );
		}
	}
}
endif;
